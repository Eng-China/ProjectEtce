    ...
    show += ` <div class="col-12 col-sm-6 col-md-3">
      <div class="card h-100">
        ...
        <div class="card-body">
          ...
          <p class="card-text">${pro.description.substring(0,20)}</p>
          <p class="fw-bold text-success">${pro.price}$</p>
          <button  type="button" onclick="AddtoCart(${pro.id})" class="btn btn-primary w-100">Add to cart</button>
        </div>
      </div>
    </div>`
...
fetch("https://eng-china.github.io/Shop/data.json")
  .then(res => res.json())
  .then(item => {
    cart = item
    Displayproducts()
...
const Updatetocart = () => {
  ...
  show += `
    <div class="cart-summary border-top pt-3">
      ...
      <button onclick="CheckOut()" class="btn btn-primary w-100 mt-3">Proceed to Checkout</button>
    </div>`;
